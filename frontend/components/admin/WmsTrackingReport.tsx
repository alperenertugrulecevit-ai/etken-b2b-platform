...
type SearchParams=Promise<{startDate?:string;endDate?:string;productCode?:string;orderNumber?:string;personnel?:string}>;
...
function Filters({kind,startDate,endDate,productCode,orderNumber,personnel}:{kind:Kind;startDate:string;endDate:string;productCode:string;orderNumber:string;personnel:string}){
 const receiving=kind==="receiving";
 return <form className="mt-7 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
...
export default async function WmsTrackingReport({kind,searchParams}:{kind:Kind;searchParams:SearchParams}){
 const q=await searchParams,startDate=q.startDate?.trim()??"",endDate=q.endDate?.trim()??"",productCode=q.productCode?.trim()??"",orderNumber=q.orderNumber?.trim()??"",personnel=q.personnel?.trim()??"";
 ...
 if(receiving){
  ...
  return <section className="p-4 sm:p-6 lg:p-10"><h1 className="text-3xl font-bold">Giriş Takip Raporu</h1><p className="mt-2 text-slate-500">Mal kabul giriş okutmalarını sipariş, ürün ve personel bazında takip edin.</p><Filters kind={kind} startDate={startDate} endDate={endDate} productCode={productCode} orderNumber={orderNumber} personnel={personnel}/>
   <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table className="w-full min-w-[1100px]"><thead><tr>{["Giriş Okutma Tarihi","Personel","Giriş Sipariş No","THM","Ürün Kodu","Ürün Tanımı","Miktar"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead><tbody>
 ...
 const rows=await prisma.pickingRecord.findMany({where,orderBy:{createdAt:"desc"},include:{order:{select:{orderNumber:true}},product:{select:{code:true,name:true}},sourceHandlingUnit:{select:{barcode:true,location:{select:{code:true}}}},targetHandlingUnit:{select:{barcode:true}}}});
 const filtered=personnel?[]:rows;
 const total=filtered.reduce((s,r)=>s+r.quantity,0);
 return <section className="p-4 sm:p-6 lg:p-10"><h1 className="text-3xl font-bold">Yapılan Toplama Detay Raporu</h1><p className="mt-2 text-slate-500">Tamamlanan toplama hareketlerini sipariş, ürün, THM ve lokasyon bazında inceleyin.</p><Filters kind={kind} startDate={startDate} endDate={endDate} productCode={productCode} orderNumber={orderNumber} personnel={personnel}/>
  ...
  <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table className="w-full min-w-[1400px]"><thead><tr>{["Toplama Tarihi","Personel","Sipariş No","Adres","Lokasyon THM","Toplama THM","Ürün Kodu","Ürün Tanımı","Miktar"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead><tbody>
  {filtered.map(r=><tr key={r.id}><td className={td}>{fmt(r.createdAt)}</td><td className={td}>-</td><td className={td}>{r.order.orderNumber}</td><td className={td}>{r.sourceHandlingUnit.location?.code??"-"}</td><td className={td}>{r.sourceHandlingUnit.barcode}</td><td className={td}>{r.targetHandlingUnit.barcode}</td><td className={td}>{r.product.code}</td><td className={td}>{r.product.name}</td><td className={td}>{n(r.quantity)}</td></tr>)}
  {filtered.length===0?<tr><td colSpan={9} className="p-10 text-center text-slate-500">Filtreye uygun toplama kaydı bulunamadı.</td></tr>:<tr className="bg-slate-100 font-bold"><td className={td}>Toplam</td><td className={td}>-</td><td className={td}>{n(new Set(filtered.map(r=>r.order.orderNumber)).size)}</td><td className={td}>{n(new Set(filtered.map(r=>r.sourceHandlingUnit.location?.code).filter(Boolean)).size)}</td><td className={td}>{n(new Set(filtered.map(r=>r.sourceHandlingUnit.barcode)).size)}</td><td className={td}>{n(new Set(filtered.map(r=>r.targetHandlingUnit.barcode)).size)}</td><td className={td}>{n(new Set(filtered.map(r=>r.product.code)).size)}</td><td className={td}>-</td><td className={td}>{n(total)}</td></tr>}</tbody></table></div></section>
}