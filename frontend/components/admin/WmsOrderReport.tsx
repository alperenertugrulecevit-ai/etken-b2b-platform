...
type SearchParams = Promise<{ startDate?: string; endDate?: string; orderNumber?: string; status?: string }>;
...
function Filters({title,orderLabel,startDate,endDate,orderNumber,status,shipment}:{title:string;orderLabel:string;startDate:string;endDate:string;orderNumber:string;status:string;shipment:boolean}){
...
export default async function WmsOrderReport({kind,searchParams}:{kind:ReportKind;searchParams:SearchParams}){
 const q=await searchParams; const startDate=q.startDate?.trim()??""; const endDate=q.endDate?.trim()??""; const orderNumber=q.orderNumber?.trim()??""; const status=q.status?.trim()??"";
 ...
 if(shipment){
  ...
  if(kind==="shipment-summary"){
   ...
   return <section className="p-4 sm:p-6 lg:p-10"><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} shipment={shipment}/>
