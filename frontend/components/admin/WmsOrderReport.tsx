...
type SearchParams = Promise<{ startDate?: string; endDate?: string; orderNumber?: string; status?: string; productCode?: string; companyCode?: string; companyName?: string }>;
...
function Filters({title,orderLabel,startDate,endDate,orderNumber,status,productCode,shipment,companyCode,companyName}:{title:string;orderLabel:string;startDate:string;endDate:string;orderNumber:string;status:string;productCode:string;shipment:boolean;companyCode:string;companyName:string}){
...
export default async function WmsOrderReport({kind,searchParams}:{kind:ReportKind;searchParams:SearchParams}){
 const q=await searchParams; const startDate=q.startDate?.trim()??""; const endDate=q.endDate?.trim()??""; const orderNumber=q.orderNumber?.trim()??""; const status=q.status?.trim()??""; const productCode=q.productCode?.trim()??""; const companyCode=q.companyCode?.trim()??""; const companyName=q.companyName?.trim()??"";
 ...
 if(shipment){
  ...
  const queriedOrders=await prisma.order.findMany({where,orderBy:{orderDate:"desc"},include:{customer:{select:{customerCode:true,companyName:true}},items:{where:itemWhere,orderBy:{id:"asc"}},fulfillment:true,shippingHandlingUnitOrders:{include:{shippingHandlingUnit:{select:{status:true,closedAt:true,shippedAt:true}}}}}});
  ...
  if(kind==="shipment-summary"){
   ...
   return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-shipment-summary-table" fileName="sevk-siparis-durum-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment} companyCode={companyCode} companyName={companyName}/>
    <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-shipment-summary-table" className="w-full min-w-[1050px]"><thead><tr>
     {["Sipariş Oluşturma Tarihi","Sipariş No","Firma Kodu","Firma İsmi","Sipariş Miktarı","Toplama Miktarı","Toplama Farkı","Paketleme Miktarı","Paketleme Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}
    </tr></thead><tbody>{rows.map(({o,ordered,picked,packed})=><tr key={o.id} className="hover:bg-slate-50"><td className={td}>{fmtDate(o.orderDate)}</td><td className={td+" font-bold text-blue-900"}>{o.orderNumber}</td><td className={td}>{o.customer.customerCode}</td><td className={td}>{o.customer.companyName}</td><td className={td}>{n(ordered)}</td><td className={td}>{n(picked)}</td><td className={td}>{n(Math.max(ordered-picked,0))}</td><td className={td}>{n(packed)}</td><td className={td}>{n(Math.max(picked-packed,0))}</td><td className={td}>{operationalLabel(o.operationalStatus)}</td></tr>)}
    {rows.length===0?<tr><td colSpan={10} className="p-10 text-center text-slate-500">Filtreye uygun sevk siparişi bulunamadı.</td></tr>:<tr className="bg-slate-100 font-bold"><td className={td}>Toplam</td><td className={td}>{n(rows.length)} sipariş</td><td className={td}>-</td><td className={td}>-</td><td className={td}>{n(totals.ordered)}</td><td className={td}>{n(totals.picked)}</td><td className={td}>{n(Math.max(totals.ordered-totals.picked,0))}</td><td className={td}>{n(totals.packed)}</td><td className={td}>{n(Math.max(totals.picked-totals.packed,0))}</td><td className={td}>-</td></tr>}</tbody></table></div></section>
  }
  ...
  return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-shipment-detail-table" fileName="sevk-siparis-detay-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment} companyCode={companyCode} companyName={companyName}/>
   <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-shipment-detail-table" className="w-full min-w-[1500px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Sipariş No","Firma Kodu","Firma İsmi","Kalem No","Ürün Kodu","Ürün Tanımı","Sipariş Miktarı","Toplama Miktarı","Toplama Farkı","Paketleme Miktarı","Paketleme Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
   <tbody>{rows.map(({o,x,line})=><tr key={x.id} className="hover:bg-slate-50"><td className={td}>{fmtDate(o.orderDate)}</td><td className={td+" font-bold text-blue-900"}>{o.orderNumber}</td><td className={td}>{o.customer.customerCode}</td><td className={td}>{o.customer.companyName}</td><td className={td}>{line}</td><td className={td}>{x.productCode}</td><td className={td+" max-w-[360px] whitespace-normal"}>{x.productName}</td><td className={td}>{n(x.quantity)}</td><td className={td}>{n(x.pickedQuantity)}</td><td className={td}>{n(Math.max(x.quantity-x.pickedQuantity,0))}</td><td className={td}>{n(x.packedQuantity)}</td><td className={td}>{n(Math.max(x.pickedQuantity-x.packedQuantity,0))}</td><td className={td}>{operationalLabel(o.operationalStatus)}</td></tr>)}
   {rows.length===0?<tr><td colSpan={13} className="p-10 text-center text-slate-500">Filtreye uygun sevk sipariş detayı bulunamadı.</td></tr>:<tr className="bg-slate-100 font-bold"><td className={td}>Toplam</td><td className={td}>-</td><td className={td}>-</td><td className={td}>-</td><td className={td}>{n(rows.length)}</td><td className={td}>-</td><td className={td}>-</td><td className={td}>{n(totals.ordered)}</td><td className={td}>{n(totals.picked)}</td><td className={td}>{n(Math.max(totals.ordered-totals.picked,0))}</td><td className={td}>{n(totals.packed)}</td><td className={td}>{n(Math.max(totals.picked-totals.packed,0))}</td><td className={td}>-</td></tr>}</tbody></table></div></section>
 }
 ...
 const orders=await prisma.purchaseOrder.findMany({where,orderBy:{orderDate:"desc"},include:{supplier:{select:{name:true,taxNumber:true}},items:{where:receiptItemWhere,orderBy:{id:"asc"}}}});
 if(kind==="receipt-summary"){
  ...
  return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-receipt-summary-table" fileName="giris-siparis-durum-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment} companyCode={companyCode} companyName={companyName}/>
   <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-receipt-summary-table" className="w-full min-w-[1000px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Satın Alma Sipariş No","Tedarikçi Kodu","Tedarikçi Adı","Sipariş Miktarı","Giriş Miktarı","Giriş Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
   <tbody>{rows.map(({o,ordered,received})=><tr key={o.id} className="hover:bg-slate-50"><td className={td}>{fmtDate(o.orderDate)}</td><td className={td+" font-bold text-blue-900"}>{o.purchaseNumber}</td><td className={td}>{o.supplier.taxNumber??"-"}</td><td className={td}>{o.supplier.name}</td><td className={td}>{n(ordered)}</td><td className={td}>{n(received)}</td><td className={td}>{n(Math.max(ordered-received,0))}</td><td className={td}>{receiptStatus(o.status)}</td></tr>)}
 ...
 return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-receipt-detail-table" fileName="giris-siparis-detay-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment} companyCode={companyCode} companyName={companyName}/>
  <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-receipt-detail-table" className="w-full min-w-[1350px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Satın Alma Sipariş No","Tedarikçi Kodu","Tedarikçi Adı","Kalem No","Ürün Kodu","Ürün Tanımı","Sipariş Miktarı","Giriş Miktarı","Giriş Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
