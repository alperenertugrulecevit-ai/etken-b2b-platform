...
export default async function WmsOrderReport({kind,searchParams}:{kind:ReportKind;searchParams:SearchParams}){
 ...
 if(shipment){
  ...
  const queriedOrders=await prisma.order.findMany({where,orderBy:{orderDate:"desc"},include:{items:{where:itemWhere,orderBy:{id:"asc"}},fulfillment:true,shippingHandlingUnitOrders:{include:{shippingHandlingUnit:{select:{status:true,closedAt:true,shippedAt:true}}}}}});
  ...
  if(kind==="shipment-summary"){
   ...
   return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-shipment-summary-table" fileName="sevk-siparis-durum-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment}/>
  ...
  return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-shipment-detail-table" fileName="sevk-siparis-detay-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment}/>
   <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-shipment-detail-table" className="w-full min-w-[1500px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Sipariş No","Kalem No","Ürün Kodu","Ürün Tanımı","Sipariş Miktarı","Toplama Miktarı","Toplama Farkı","Paketleme Miktarı","Paketleme Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
 ...
 const orders=await prisma.purchaseOrder.findMany({where,orderBy:{orderDate:"desc"},include:{supplier:{select:{name:true}},items:{where:receiptItemWhere,orderBy:{id:"asc"}}}});
 if(kind==="receipt-summary"){
  ...
  return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-receipt-summary-table" fileName="giris-siparis-durum-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment}/>
   <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-receipt-summary-table" className="w-full min-w-[1000px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Satın Alma Sipariş No","Tedarikçi","Sipariş Miktarı","Giriş Miktarı","Giriş Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
 ...
 return <section className="p-4 sm:p-6 lg:p-10"><div className="flex justify-end"><ExcelTableExportButton tableId="wms-receipt-detail-table" fileName="giris-siparis-detay-raporu.csv"/></div><Filters title={m.title} orderLabel={m.orderLabel} startDate={startDate} endDate={endDate} orderNumber={orderNumber} status={status} productCode={productCode} shipment={shipment}/>
  <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><table id="wms-receipt-detail-table" className="w-full min-w-[1350px]"><thead><tr>{["Sipariş Oluşturma Tarihi","Satın Alma Sipariş No","Kalem No","Tedarikçi","Ürün Kodu","Ürün Tanımı","Sipariş Miktarı","Giriş Miktarı","Giriş Farkı","Sipariş Durumu"].map(x=><th key={x} className={th}>{x}</th>)}</tr></thead>
