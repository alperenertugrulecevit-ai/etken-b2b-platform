...
export default function HomeHeroSlider({
  ...
  return (
    <section className="bg-white">
      <div className="mx-auto max-w-[1160px] px-4 py-4 sm:px-6 lg:px-8">
        <div
          className="relative"
          onMouseEnter={() =>
            setIsPaused(
        ...
        >
<div
  className={`relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r ${slide.backgroundClassName} shadow-sm`}
>
  <div className="grid min-h-[235px] items-center gap-4 px-7 py-5 md:grid-cols-[minmax(0,1fr)_minmax(320px,0.82fr)] lg:min-h-[250px] lg:px-9">
    <div className="relative z-10 max-w-[570px]">
      <span
        className={`inline-flex rounded-full bg-white/80 px-3 py-1 text-[10px] font-black uppercase tracking-[0.08em] shadow-sm ${slide.eyebrowClassName}`}
      >
        {slide.eyebrow}
      </span>

      <h1 className="mt-3 text-[25px] font-black leading-[1.08] tracking-tight text-[#172435] sm:text-[29px] lg:text-[32px]">
        {slide.title}
        ...
        <span className="mt-1 block text-[#EF4B23]">
          {slide.highlight}
                ...
                <p className="mt-3 max-w-[520px] text-[13px] leading-5 text-slate-600 sm:text-sm">
                  {
                    slide.description
                  }
                </p>
                ...
                <div className="mt-5 flex flex-wrap gap-2.5">
                  ...
                  slide.secondaryHref ? (
                    <Link
                      ...
                      className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white/80 px-5 text-xs font-black text-[#202B38] transition hover:border-[#EF4B23] hover:text-[#EF4B23]"
                    >
                      {
                        slide.secondaryLabel
                      }
              ...
              <div className="relative hidden h-[220px] items-center justify-center md:flex">
                {slide.product ? (
                  <Link
                  ...
                  >
                    <ProductImage
                      ...
                      className="h-[175px] w-full max-w-[320px] rounded-xl bg-transparent object-contain p-3 transition duration-500 group-hover:scale-[1.03]"
                      fallbackClassName="h-[200px] w-full max-w-[380px] rounded-xl text-5xl"
                    />

                    <div className="absolute bottom-2 right-3 max-w-[250px] rounded-xl border border-white/70 bg-white/90 px-3 py-2 shadow-md backdrop-blur">
                      ...
                      <p className="mt-0.5 line-clamp-2 text-[11px] font-bold leading-4 text-slate-800">
                        {
                          slide.product.name
                        }
                      </p>
                  ...
                  <div className="flex h-[180px] w-[300px] items-center justify-center rounded-2xl border border-slate-200 bg-white/70 text-6xl shadow-sm">
                    📦
                  </div>
                )}

                {slide.statValue ? (
                  <div className="absolute right-2 top-1 rounded-xl border border-slate-200 bg-white/90 px-4 py-3 text-center shadow-sm backdrop-blur">
                    ...
                    <p className="mt-0.5 text-lg font-black text-[#172435]">
                      {
                        slide.statValue
                      }
                    </p>
          ...
          <div className="absolute bottom-3 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-white/75 px-2.5 py-1.5 shadow-sm backdrop-blur">
            {slides.map(
              (
                item,
                index,
