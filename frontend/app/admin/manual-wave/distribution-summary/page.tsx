      ...
      <div className="mx-auto max-w-7xl space-y-5 p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex gap-2">
            ...
            <Link href={`/admin/manual-wave/performance?${reportParams.toString()}`} className="rounded-lg border border-slate-200 px-5 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">Saatlik Performans</Link>
          </div>
          <Link href={`/admin/manual-wave/distribution-summary/performance-export?${reportParams.toString()}`} className="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-bold text-white hover:bg-emerald-700">Excel İndir</Link>
        </div>
