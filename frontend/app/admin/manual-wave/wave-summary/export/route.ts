...
export async function GET(req:NextRequest){
 ...
 const rows=w.distributions.flatMap(d=>d.lines.filter(l=>l.plannedQuantity>l.packedQuantity).map(l=>({storeCode:d.customerCode||String(d.sequenceNumber),storeName:d.customerName,productCode:l.productCode,barcode:l.productBarcode||"",name:l.productName,planned:l.plannedQuantity,packed:l.packedQuantity,remaining:l.plannedQuantity-l.packedQuantity}))).filter(r=>(!store||r.storeCode.toLowerCase().includes(store))&&(!product||r.productCode.toLowerCase().includes(product))&&(!barcode||r.barcode.toLowerCase().includes(barcode))&&(!name||r.name.toLowerCase().includes(name)));
