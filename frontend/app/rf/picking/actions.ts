...
function createFullLocationCode({
  ...
  return [code, section, level, bin].filter(Boolean).join("-").toUpperCase();
}
...
function canPickOrder(status: OrderStatus) {
  return (
...
function canUseSourceStatus(status: HandlingUnitStatus) {
  return (
...
function canUseTargetStatus(status: HandlingUnitStatus) {
  return (
    status === HandlingUnitStatus.OPEN ||
...
export async function rfPickOrderItem(
  ...
  const locationBarcode = normalizeValue(formData.get("locationBarcode"));
  ...
  if (!targetBarcode) {
    return createErrorState(
      "Hedef Toplama THM veya Sevk THM barkodunu okutun.",
    );
  }

  if (!locationBarcode) {
    return createErrorState("Kaynak lokasyon barkodunu okutun.");
  }

  if (!sourceBarcode) {
    return createErrorState("Kaynak stok koli veya paletini okutun.");
  }

  if (!productBarcode) {
    return createErrorState("Toplanacak ürün barkodunu okutun.");
  }
  ...
  try {
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        const pickingFlow = await FulfillmentService.resolvePickingFlow(
          tx,
          order.id,
        );
        ...
        const directShippingUnit =
          pickingFlow.flowType === OrderFulfillmentFlow.WAVE
            ? null
            : await FulfillmentService.prepareDirectShippingUnit(tx, {
                orderId: order.id,
                targetBarcode,
                actor: {
        ...
        if (!targetUnit) {
          throw new Error(`${targetBarcode} barkodlu hedef THM bulunamadı.`);
        }
        ...
        if (sourceUnit.purpose !== HandlingUnitPurpose.STOCK) {
          throw new Error(
        ...
        if (sourceUnit.assignedOrderId !== null) {
          throw new Error(
        ...
        if (sourceUnit.assignedWaveId !== null) {
          throw new Error(
            `${sourceUnit.barcode} bir Wave'e bağlıdır. ` +
              "Normal stok kaynağı olarak kullanılamaz.",
          );
        }

        if (!canUseSourceStatus(sourceUnit.status)) {
          throw new Error(
        ...
        if (!sourceUnit.warehouse.isActive) {
          throw new Error(
            `${sourceUnit.barcode} pasif bir depoda bulunmaktadır.`,
          );
        }

        if (!sourceUnit.location.isActive) {
          throw new Error(
        ...
        const expectedLocationCode = createFullLocationCode({
          ...
          section: sourceUnit.location.section,

          level: sourceUnit.location.level,

          bin: sourceUnit.location.bin,
        });

        if (expectedLocationCode !== locationBarcode) {
          throw new Error(
        ...
        const expectedTargetPurpose = isWavePicking
          ? HandlingUnitPurpose.PICKING
          : HandlingUnitPurpose.SHIPPING;

        if (targetUnit.purpose !== expectedTargetPurpose) {
          throw new Error(
            isWavePicking
              ? `${targetUnit.barcode} bir Toplama THM değildir.`
              : `${targetUnit.barcode} bir Sevk THM değildir.`,
          );
        }

        if (!canUseTargetStatus(targetUnit.status)) {
          throw new Error(
            `${targetUnit.barcode} hedef THM ürün kabul etmeye uygun değildir.`,
          );
        }
        ...
        if (
          isWavePicking &&
          (targetUnit.assignedWaveId !== pickingFlow.waveId ||
            targetUnit.assignedOrderId !== null)
        ) {
          throw new Error(
            `${targetUnit.barcode} Toplama THM, ${pickingFlow.waveNo ?? "seçilen"} Wave'e bağlı değildir.`,
          );
        }
        ...
        if (
          ...
          throw new Error(
            `${targetUnit.barcode} Sevk THM, ${order.orderNumber} siparişine bağlı değildir.`,
          );
        }
        ...
        const orderPickedQuantity = updatedOrderItems.reduce(
          (total, item) => total + Math.min(item.pickedQuantity, item.quantity),
          0,
        );

        const orderRemainingQuantity = Math.max(
          0,
        ...
        const pickingRecord = await tx.pickingRecord.create({
          data: {
            ...
            sourceHandlingUnitId: sourceUnit.id,

            targetHandlingUnitId: targetUnit.id,
            ...
            targetQuantityAfter: targetProductItem.quantity,
          },
        ...
        await tx.wmsOperationLog.create({
          data: {
            ...
            entityType: "PICKING_RECORD",

            entityId: pickingRecord.id,
            ...
            warehouseCode: sourceUnit.warehouse?.code ?? null,

            sourceLocationId: sourceUnit.locationId,

            sourceLocationCode: expectedLocationCode,

            targetLocationId: targetUnit.locationId,
            ...
            description:
              ...
              `${targetUnit.barcode} ${targetTypeLabel}'sine aktarıldı.`,
            ...
            metadata: {
              ...
              waveNo: pickingFlow.waveNo,

              shippingHandlingUnitId: directShippingUnit?.id ?? null,

              packageSequence: directShippingUnit?.packageSequence ?? null,

              pickingRecordId: pickingRecord.id,
              ...
              sourceProductQuantityBefore: sourceItem.quantity,

              sourceProductQuantityAfter: sourceQuantityAfter,

              sourceTotalQuantityAfter: sourceTotalQuantity,

              targetProductQuantityAfter: targetProductItem.quantity,

              targetTotalQuantityAfter: targetTotalQuantity,

              lineOrderedQuantity: updatedOrderItem.quantity,

              linePickedQuantityAfter: updatedOrderItem.pickedQuantity,

              lineRemainingQuantityAfter: Math.max(
                0,
              ...
              orderPickedQuantityAfter: orderPickedQuantity,

              orderRemainingQuantityAfter: orderRemainingQuantity,

              pickingCompleted,

              recordedAt: pickingRecord.createdAt.toISOString(),
            },
        ...
        const progressPercentage =
          orderTotalQuantity > 0
            ? Math.min(
                100,
                Math.round((orderPickedQuantity / orderTotalQuantity) * 100),
              )
            : 0;
        ...
        return {
          ...
          targetQuantityAfter: targetTotalQuantity,
    ...
    return {
      ...
      message:
        ...
        `${result.targetBarcode} ${result.targetTypeLabel}'sine aktarıldı.` +
        (result.waveNo
          ? ` Wave: ${result.waveNo}.`
      ...
      orderStatus: getOrderStatusLabel(result.orderStatus),
      ...
      sourceQuantityAfter: result.sourceQuantityAfter,

      targetUnitId: result.targetUnitId,

      targetBarcode: result.targetBarcode,

      targetQuantityAfter: result.targetQuantityAfter,

      pickedQuantity: result.pickedQuantity,

      linePickedQuantity: result.linePickedQuantity,

      lineRemainingQuantity: result.lineRemainingQuantity,

      orderPickedQuantity: result.orderPickedQuantity,

      orderTotalQuantity: result.orderTotalQuantity,

      orderRemainingQuantity: result.orderRemainingQuantity,
