...
export default async function RFConsolidationPage({searchParams}:{searchParams:Promise<{done?:string}>}) {
  ...
  const points=await prisma.consolidationPoint.findMany({where:{isActive:true},orderBy:[{warehouseId:"asc"},{code:"asc"}]});
  return <section>
    ...
    <div className="grid gap-4">{tasks.map(t=>{const ready=t.status==="READY"; const taskPoints=points.filter(p=>p.warehouseId===t.warehouseId); return <article key={t.id} className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      {ready&&<form action={completeConsolidation} className="mt-4 flex flex-wrap gap-2"><input type="hidden" name="taskId" value={t.id}/><select name="pointId" required className="min-w-56 flex-1 rounded-xl border p-3"><option value="">Konsolidasyon noktası seç</option>{taskPoints.map(p=><option key={p.id} value={p.id}>{p.code} - {p.name}</option>)}</select><button className="rounded-xl bg-blue-900 px-5 py-3 font-black text-white">Konsolidasyonu Tamamla</button></form>}
    </article>})}</div>
