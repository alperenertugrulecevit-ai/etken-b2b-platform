...
export default async function RFWarehouseProductTransferPage() {
  ...
  return <section className="space-y-4">
    <div className="flex items-start justify-between gap-3">
      <div><p className="text-xs font-black uppercase tracking-wider text-blue-700">RF · Transfer İşlemleri</p><h1 className="mt-1 text-2xl font-black">Depolararası Transfer Ürün</h1></div>
    ...
    <form action={createRfWarehouseTransfer} className="rounded-2xl border bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-4">
        <label className="text-sm font-black">1. Kaynak THM
        ...
        <datalist id="source-units">{units.map(u=><option key={u.id} value={u.barcode}>{u.warehouse?.code} / {u.location?loc(u.location):"-"}</option>)}</datalist>
        <label className="text-sm font-black">2. Ürün
          <select name="productId" required className="mt-2 w-full rounded-xl border p-4">
        ...
        <label className="text-sm font-black">3. Transfer Miktarı
          <input name="quantity" type="number" min="1" step="1" required inputMode="numeric" className="mt-2 w-full rounded-xl border p-4 text-lg font-black" />
        </label>
        <label className="text-sm font-black">4. Hedef Depo
        ...
        <label className="text-sm font-black">5. Hedef Lokasyon
        ...
        <label className="text-sm font-black">6. Hedef THM
          <input name="targetHandlingUnitBarcode" list="target-units" required autoComplete="off" placeholder="Hedef THM barkodunu okutun" className="mt-2 w-full rounded-xl border-2 border-cyan-300 p-4 text-lg font-black uppercase" />
        </label>
        <datalist id="target-units">{units.map(u=><option key={u.id} value={u.barcode}>{u.warehouse?.code} / {u.location?loc(u.location):"-"}</option>)}</datalist>
        ...
        <button className="rounded-xl bg-blue-900 py-4 text-lg font-black text-white">ÜRÜN TRANSFERİNİ TAMAMLA</button>
      </div>
    </form>
  </section>;
}
